...
class BmgReposicaoCartaoBeneficio(Bank):
    def __init__(self, name = "BMG", num = 701, type = "excel"):
        super().__init__(name, num, type)

    def readArchive(self, df):
        try:
            logger.info("Inicio do processo de leitura do df-BMG Reposicao Cartao Beneficio")
            df = pd.read_csv(df, sep=';', encoding='latin-1')
            df = df.rename(columns={'Adesão': 'Adesao'})
            df = df[pd.notna(df['Adesao'])]
            logger.info("Lido o arquivo do BMG Reposicao Cartao Beneficio")
            return df
        except Exception:
        ...
        try:
            ...
            if "Comissão Ato ou Diferido" in df.columns:
                infos ={
                    ...
                    "Data da Liquidação": "DAT_CREDITO",
